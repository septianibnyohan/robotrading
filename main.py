...
def main():
    ...
    # Configure logging level dynamically based on --debug flag
    ...
    try:
        while True:
            ...
            # Calculate indicators on the full dataset (including the forming bar) for real-time tracking
            df_realtime = strategy.calculate_indicators(df)
            if df_realtime.empty:
                time.sleep(2.0)
                continue
            realtime_bar = df_realtime.iloc[-1]
