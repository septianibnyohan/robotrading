...
const LayerChart = ({ activeSession, currentPrice, config }) => {
  ...
  if (!activeSession.first_entry_price) {
    return (
      <div style={{
        height: '280px',
        width: '100%',
        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{opacity: 0.5, stroke: 'var(--secondary)'}}>
          <circle cx="12" cy="12" r="10"/>
          <polyline points="12 6 12 12 16 14"/>
        </svg>
        <span style={{ color: 'var(--text-white)', fontWeight: '600' }}>{activeSession.symbol} Grid Monitor</span>
        <span style={{ fontSize: '0.8rem' }}>Active: Awaiting first trade signal...</span>
      </div>
    );
  }
