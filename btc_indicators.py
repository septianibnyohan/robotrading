...
from btc_config import EMA_FAST, EMA_MED, EMA_SLOW, H1_EMA_PERIOD, RSI_PERIOD, ATR_PERIOD, ADX_PERIOD, VOL_EMA_PERIOD
...
def compute_emas(df):
    ...
    df['ema_21'] = df['close'].ewm(span=EMA_MED, adjust=False).mean()
...
def compute_adx(df, tr):
    ...
    tr_smoothed = tr.ewm(alpha=1/ADX_PERIOD, adjust=False).mean()
    plus_di = 100 * pd.Series(plus_dm_clean).ewm(alpha=1/ADX_PERIOD, adjust=False).mean() / (tr_smoothed + 1e-10)
    minus_di = 100 * pd.Series(minus_dm_clean).ewm(alpha=1/ADX_PERIOD, adjust=False).mean() / (tr_smoothed + 1e-10)
    
    dx = 100 * (plus_di - minus_di).abs() / (plus_di + minus_di + 1e-10)
    df['adx_14'] = dx.ewm(alpha=1/ADX_PERIOD, adjust=False).mean()
    return df
...
def calculate_m5_indicators(df):
    ...
    df['volume_ema_10'] = df['tick_volume'].ewm(span=VOL_EMA_PERIOD, adjust=False).mean()
    return df

def calculate_h1_indicators(df):
    """Calculates H1 indicators (EMA 50 and slope)."""
    df = df.sort_values('time').copy()
    df['ema_50'] = df['close'].ewm(span=H1_EMA_PERIOD, adjust=False).mean()
